...
### 2026/02/13(4차 CSS 선택자 작성 후)
* html
    * ul에 이름 주고 li에는 이름 주지 말기
    * 태그에 이름을 줄 때 통일성 중요 (이름을 준 태그,이름 유무 등등)
    * men women 외 다른 메뉴들도 depth 작성
    * gnb 이름 통일하기
    * depth 안 ul div 그룹으로 묶기
* css
    * 중복되는 태그 확인 후 `>` 추가하기 (이름을 준 태그도 태그 자체가 선택자로 되어있을 때 해당됨)
## 자주 실수하는 부분
* 속성 값 빼먹지 말기!!제발.
    * **`<a href="#">`**
    * `<form action="" method="">`
    * `<input type="" name="">`
    * `<button type="" id="">`
* 진행 방향이 다르거나 여백이 크게 있는 경우에는 그룹 태그로 묶어주기
    * div, ul, ol , dl, span 등
* 태그를 열지 않아도 안에 있는 내용을 알 수 있도록 이름 작성하기 
